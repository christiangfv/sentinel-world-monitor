...
function formatLocation(region, comuna) {
...
exports.fetchCSNEvents = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        for (const event of events) {
            try {
                ...
                if (!event.id || !event.magnitude || !event.lat || !event.lng) {
                    firebase_functions_1.logger.warn(`⚠️ Evento CSN sin datos completos: ${JSON.stringify(event)}`);
                    skippedCount++;
                    continue;
                }
                ...
                const eventData = {
                    ...
                    eventTime: firestore_1.Timestamp.fromDate(event.time),
                ...
                firebase_functions_1.logger.error(`❌ Error procesando evento CSN ${event.id}:`, error);
                continue;
            }
        }
        // Ejecutar batch
        ...
        firebase_functions_1.logger.error('❌ Error en fetchCSNEvents:', error);
        throw error;
    }
});
// Parser simplificado de HTML CSN
function parseCSNHTML(htmlText) {
    const events = [];
    try {
        ...
        if (!tableMatch) {
            firebase_functions_1.logger.warn('No se encontró tabla de sismos en HTML del CSN');
            return events;
        }
        ...
        firebase_functions_1.logger.error('Error parsing CSN HTML:', error);
    }
    return events;
}
// Función auxiliar para estimar coordenadas en Chile basado en ubicación
function estimateChileCoordinates(locationStr) {
...
//# sourceMappingURL=fetchCSN.js.map